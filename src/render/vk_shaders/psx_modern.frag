...
// and the Vulkan fixture shader. The material maps always have a binding; the
// backend supplies 1x1 white defaults so "no map" is an identity multiply.
...
layout(push_constant) uniform Push
{
	mat4 world;
	vec4 color;
	vec4 factors;	// x = metallic, y = roughness
	vec4 emissive;	// rgb = emissive factor
} pc;
...
void main()
{
	...
	vec3 emission = pc.emissive.rgb * texture(s_emissiveMap, vUv).rgb;
	...
	for (int i = 0; i < PSYX_VK_MAX_LIGHTS; i++)
	{
		...
		{
			L = normalize(mat3(u.cameraRotation) * u.lights[i].dirType.xyz);
			radiance *= ShadowFactor(max(dot(N, L), 0.0));
		}
		else
		{
			vec3 lp = mat3(u.cameraRotation) * (u.lights[i].posRange.xyz - u.cameraPos.xyz);
			vec3 d = lp - vViewPos;
			float dist = length(d);
			L = d / max(dist, 1e-4);
